...
const _detailPresetFields = [
  'BUST', 'WAIST', 'HIP', 'SHOULDER', 'SLEEVE', 'LENGTH', 'CHEST',
  'FLARE', 'THIGH', 'KNEE', 'ANKLE', 'NECK', 'SLIT', 'ARM HOLE',
...
class _TemplateDetailScreenState extends State<TemplateDetailScreen> {
  ...
  void _onDelete(AppColorScheme c) {
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        title: Text(AppLocalizations.of(context).deleteTemplateTitle, style: GoogleFonts.poppins(fontWeight: FontWeight.bold, color: c.textDark, fontSize: 16)),
        content: Text(
          AppLocalizations.of(context).deleteTemplateConfirm(widget.template.name),
          style: GoogleFonts.poppins(fontSize: 13, color: c.gray),
        ),
        actions: [
          TextButton(
            onPressed: () => context.pop(),
            child: Text(AppLocalizations.of(context).cancel, style: GoogleFonts.poppins(color: c.gray)),
          ),
          ElevatedButton(
            ...
            onPressed: () {
              context.read<TemplateBloc>().add(DeleteTemplate(widget.template.id));
              context.pop();
            },
            child: Text(AppLocalizations.of(context).delete, style: GoogleFonts.poppins(fontSize: 13, color: Colors.white)),
          ),
        ],
      ),
    );
      ...
      listener: (context, state) {
        if (state is TemplateUpdateSuccess) {
          if (_isEditing) {
            ...
            showAppSnackBar(context, message: AppLocalizations.of(context).templateUpdated);
          }
        } else if (state is TemplateDeleteSuccess) {
          context.pop();
          showAppSnackBar(context, message: AppLocalizations.of(context).templateDeleted);
                          ...
                          BlocBuilder<TemplateBloc, TemplateState>(
                            builder: (context, state) {
                              ...
                              return AppActionBar(
                                ...
                                saveLabel: isLoading ? l10n.saving : l10n.saveChanges,
                                onEditSaveTap: () {
                                  if (isLoading) return;
                                  if (_isEditing) {
                                    _onSave(c);
  ...
  Widget _buildHeader(AppColorScheme c, bool isDark, AppLocalizations l10n) {
    return Padding(
      ...
      child: Row(
        children: [
          AppBackButton(onTap: _isEditing
              ? () => setState(() {
                    _isEditing = false;
                    _nameCtrl.text = widget.template.name;
                    _categoryCtrl.text = widget.template.category;
                    _fields = List.from(widget.template.fields);
                    _iconIndex = AppIcons.presetIcons.indexWhere((ic) => ic.codePoint == widget.template.iconCodePoint);
                    if (_iconIndex < 0) _iconIndex = 0;
                  })
              : () => context.pop()),
          ...
          Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Text(l10n.templates.toUpperCase(), style: GoogleFonts.poppins(fontSize: 11, fontWeight: FontWeight.bold, letterSpacing: 1.2, color: c.textDark.withValues(alpha: 0.6))),
              Text(_isEditing ? l10n.editTemplate : l10n.templateDetails, style: GoogleFonts.poppins(fontSize: 22, fontWeight: FontWeight.bold, color: c.textDark, height: 1.2)),
            ],
          ),
          if (_isEditing) ...[const Spacer(), TextButton(
            onPressed: () => setState(() {
              _isEditing = false;
              _nameCtrl.text = widget.template.name;
              _categoryCtrl.text = widget.template.category;
            ...
            child: Text(l10n.cancel, style: GoogleFonts.poppins(fontSize: 13, color: c.gray)),
          )],
        ],
      ),
    );
        ...
        Container(
          ...
          child: TextField(
            ...
            decoration: InputDecoration(
              ...
              hintText: _isEditing ? null : getLocalizedTemplateName(widget.template.id, widget.template.name, l10n),
        ...
        Container(
          ...
          child: _isEditing
              ? TextField(
                  ...
                  style: GoogleFonts.poppins(fontSize: 14, color: c.textDark),
                  decoration: InputDecoration(
                    prefixIcon: Icon(Icons.category_outlined, color: c.gray, size: 20),
                    border: InputBorder.none,
                    contentPadding: const EdgeInsets.symmetric(horizontal: 16, vertical: 16),
                  ),
                )
      ...
      children: [
        ...
        Container(
          ...
          child: TextField(
            ...
            style: GoogleFonts.poppins(fontSize: 14, color: c.textDark),
            decoration: InputDecoration(
              prefixIcon: Icon(Icons.currency_rupee_outlined, color: c.gray, size: 20),
              border: InputBorder.none,
              contentPadding: const EdgeInsets.symmetric(horizontal: 16, vertical: 16),
            ),
          ),
        ...
        children: _detailPresetFields.map((field) {
          ...
          return GestureDetector(
            ...
            child: AnimatedContainer(
              ...
              decoration: BoxDecoration(
                color: added ? c.colorPrimary : c.colorPrimary.withValues(alpha: 0.08),
                borderRadius: BorderRadius.circular(8),
                border: Border.all(color: added ? c.colorPrimary : c.colorPrimary.withValues(alpha: 0.2)),
              ),
              child: Row(
                mainAxisSize: MainAxisSize.min,
                children: [
                  if (added) const Padding(padding: EdgeInsets.only(right: 4), child: Icon(Icons.check, color: Colors.white, size: 11)),
                  Text(getLocalizedMeasurementField(field, l10n), style: GoogleFonts.poppins(fontSize: 11, fontWeight: FontWeight.bold, color: added ? Colors.white : c.colorPrimary)),
                ],
              ),
            ),
          );
  ...
  Widget _buildFieldsList(AppColorScheme c, bool isDark) {
    ...
    if (!_isEditing) {
      ...
      return Wrap(
        ...
        children: _fields.map((f) => Container(
          ...
          child: Text(getLocalizedMeasurementField(f, l10n), style: GoogleFonts.poppins(fontSize: 12, fontWeight: FontWeight.bold, color: c.colorPrimary)),
        )).toList(),
      );
    }
    ...
    return SizedBox(
      height: (_fields.length * 64.0).clamp(0, 400),
            ...
            child: Material(
              ...
              child: ListTile(
                ...
                title: Container(
                  ...
                  child: Text(getLocalizedMeasurementField(field, l10n), style: GoogleFonts.poppins(fontSize: 12, fontWeight: FontWeight.bold, color: c.colorPrimary)),
                ),
  ...
  Widget _buildCustomFieldInput(AppColorScheme c, bool isDark) {
    return Container(
      ...
      child: Row(
        children: [
          Icon(AppIcons.presetIcons[_iconIndex], color: c.colorPrimary, size: 28),
          ...
          GestureDetector(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(color: c.colorPrimary, shape: BoxShape.circle),
